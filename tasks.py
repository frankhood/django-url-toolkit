...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-url-toolkit runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-url-toolkit.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ url_toolkit")
...
def lint(c):
    ...
    """
    c.run("flake8 django-url-toolkit tests")
...
def release(c, bumpsize=""):
    ...
    c.run(
        'git tag -a {version} -m "New version: {version}"'.format(
            version=url_toolkit.__version__
        )
    )
    c.run("git push --tags")
